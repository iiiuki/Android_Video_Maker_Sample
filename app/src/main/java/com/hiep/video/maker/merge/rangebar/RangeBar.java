...
public class RangeBar extends View {
    ...
    private static final int SKEE_LEFT= R.mipmap.skee_left;
    private static final int SKEE_LEFT_ACTIVE=R.mipmap.skee_left_active;
    private static final int SKEE_RIGHT=R.mipmap.skee_right;
    private static final int SKEE_RIGHT_ACTIVE=R.mipmap.skee_right_active;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mRightThumb.setDuration(mRightIndex);
        mLeftThumb.setDuration(mLeftIndex);
    ...
    private void onActionUp(float x, float y) {
        if (weight==0 && mTickCount>0){
            weight=(float)getWidth() / (float)mTickCount;
            weight=weight*20F;
        }
        ...
        if (isIntecept){
            if (isLeftPress){
                mLeftIndex=mRightIndex-20;
                mLeftThumb.setX(mRightThumb.getX()-weight);
            }else {
                mRightIndex=mLeftIndex+20;
                mRightThumb.setX(mLeftThumb.getX()+weight);
            }
            if (mListener != null) {
                mListener.onWarning();
            }
...
//        }
    ...
    private void onActionMove(float x) {
        ...
        if (mRightIndex - mLeftIndex  <= 20){
            if (mLeftThumb.isPressed() && !isSwipeLeft){
                move(x);
            }else if (mRightThumb.isPressed() && !isSwipeRight){
                move(x);
            }else {
                if (mLeftThumb.isPressed()){
                    isLeftPress=true;
    ...
    private void move(float x){
        ...
        if (mLeftThumb.getX() > mRightThumb.getX()) {
            ...
            mLeftThumb.setThumbImage(SKEE_RIGHT,SKEE_RIGHT_ACTIVE);
            mRightThumb.setThumbImage(SKEE_LEFT,SKEE_LEFT_ACTIVE);
        }else {
            mLeftThumb.setThumbImage(SKEE_LEFT,SKEE_LEFT_ACTIVE);
            mRightThumb.setThumbImage(SKEE_RIGHT,SKEE_RIGHT_ACTIVE);
        }
        ...
        if (newLeftIndex != mLeftIndex || newRightIndex != mRightIndex) {
            ...
            if (mListener != null) {
                mListener.onMove(this, mLeftIndex, mRightIndex);
            }
        }
    }
    ...
    public static interface OnRangeBarChangeListener {
        ...
        public void onMove(RangeBar rangeBar, int leftThumbIndex, int rightThumbIndex);
